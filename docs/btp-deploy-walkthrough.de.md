...
> Dieser Text ist mehr ein Protokoll, was zum Deployment getan wurde und eher weniger eine kompakte Anleitung.
> Das Vorgehen ist **prinzipiell** übertragbar aber einzelne Werte können so nicht wiederverwendet werden.
...
- **Typed (kind, detail)-Klassifikator für on-prem Fehler** (PR [#69](https://github.com/Hochfrequenz/go-sap-btp-cf-template/pull/69), Issue [#66](https://github.com/Hochfrequenz/go-sap-btp-cf-template/issues/66)) — Folge des #67-Befundes: der huma-Envelope schickte vorher `detail: "on-premise call failed"` für jede `err`-Variante aus `svc.CallOnPremise`, was die Diagnose unnötig erschwerte. `btp.ClassifyOnPremError(err) (kind, detail)` ordnet jetzt `ErrDestinationNotFound` / `ErrOnPremResponseTooLarge` / `context.DeadlineExceeded` / `context.Canceled` typisierten Kinds zu (`destination_not_found`, `response_too_large`, `timeout`, `canceled`) plus `transport_error` als Fallback; der `examples/adtdiscovery`-Handler nutzt den Helper, slog bekommt `kind=` zusätzlich zum rohen `err`. Wire-Format ist Teil des öffentlichen API-Surfaces in `internal/btp/doc.go`.
...
- **On-Prem-Request an Destination-Scheme+Host gepinnt** (PR [#102](https://github.com/Hochfrequenz/go-sap-btp-cf-template/pull/102)) — CodeQL `go/request-forgery` (kritisch) hat `internal/btp/service.go` `callOnce` markiert: die Ziel-URL wurde durch String-Konkatenation des user-beeinflussten `pathSuffix` an die Destination-Basis-URL gebaut und direkt an `http.NewRequestWithContext` gegeben, ohne Host-Gleichheits-Guard (genau die Sanitiser-Form, die die Query erwartet). Praktisch war das Risiko gering — `dest.URL` kommt operator-getrusted aus dem SAP-Destination-Service, und `CallOnPremise` lehnt `..` und `%` im `pathSuffix` bereits ab — aber es fehlte das Defense-in-Depth-Pinning. Jetzt: Basis-URL via `net/url` parsen, nach dem Request-Bau `req.URL.Scheme`/`Host` gegen die Basis prüfen und sonst fail-closed; eine nicht-parsebare Destination-URL wählt nicht mehr ein unerwartetes Ziel an, sondern liefert einen Fehler. Coverage des `internal/btp`-Layers bleibt bei 92,4 % (über dem 90-%-Gate).
